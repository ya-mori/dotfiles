...
> **2026-08-13 に廃止し、2026-08-31 に `ore-improve` として再設計した**（下記）。
> 旧方式は hook が 2026-07-08 の書き換え以降サイレントに停止し、36日間気づかれなかった。
> ただし根の原因は hook の脆さではなく、**集めていたのがツール実行の羅列（593件）で、
> 読んでも次の行動が変わらなかった**こと。新方式は収集を memory に任せ、昇格だけを担う。

### 学びをルールへ昇格させる

セッションで得た学びは memory に溜まる。**溜めるだけでは行動は変わらない**ので、
`ore-improve` で恒久ルールへ昇格させる。

| | 収集 | 置き場 |
|---|---|---|
| Claude | ネイティブ memory（自動） | `~/.claude/projects/*/memory/*.md` |
| Codex | `core.md`「Learning Capture Guidelines」に従って自分で書く | `~/.ai_workspace/memory/*.md` |

Codex にネイティブの蓄積先が無い（`~/.codex/memories_1.sqlite` は空で、markdown の
読み書き口も無い）ため、共有の置き場を1つ設けて**両ツールの学びを同じ形式にそろえている**。

```bash
# 未昇格の学びを数える（ore-task の完了報告でも出る）
grep -L "^promoted:" ~/.claude/projects/*/memory/*.md ~/.ai_workspace/memory/*.md 2>/dev/null \
  | grep -v "MEMORY.md" | wc -l
```

昇格したファイルは frontmatter に `promoted: <日付> <昇格先>` が入り、次回の抽出から外れる。
**ファイルは消さない**（なぜ昇格させたかの経緯が残る）。

**常駐プロセスも hook も持たない設計にしてある。** 起動の導線は `ore-task` の完了報告のみ。
自動で回らない代わりに、**起きうる失敗が「起動していない」だけ**になり、旧方式のような
サイレント停止が構造的に発生しない。
...
| L2 混入の検出 | `.githooks/pre-commit` | **public 側のファイル**に L2 の語が書かれたら staged 段階で止める。検出語は `.githooks/private-l2-patterns`（それ自体が L2 なので git 追跡外） |

**この2つは守る対象が違う。** `.gitignore` は「private というファイルが git に入ること」を防ぎ、
`pre-commit` は「public 側のファイルの中身に L2 が混ざること」を防ぐ。
**前者だけでは後者を一切カバーできない**（下記「なぜフックを足したか」）。
...
### public 側の例示はプレースホルダで書く

L1 のファイル（このリポジトリに入るもの）に実例を書くときは、固有名詞をプレースホルダへ置き換える。

| プレースホルダ | 何を指すか |
|---|---|
| `{ID prefix}` | タスク台帳の ID prefix |
| `<repo>` | リポジトリ名 |
| `プロダクトA` / `プロダクトB` | プロダクト名 |
| `媒体A` | 媒体名 |
| `施策Y` / `外部連携X` | 社内施策・連携先の名前 |
| `担当者P` | 実名 |

実物との対応表は private 側に持つ（`docs/task-system/private.md` §2）。

### なぜフックを足したか（2026-08-18）

**ドキュメントは「実例があるほど説得力が出る」ため、構造的に L2 が入りやすい。**
スキルの提示フォーマットを説明する例として実タスクの ID・プロダクト名・実名を書いてしまい、
`git filter-repo` で**全履歴を書き換えて force push する**事態になった。

- 安全網が `.gitignore` の1本だけで、**public 側ファイルの中身は誰も見ていなかった**
- 気づいたのは push 直前の手動確認。**偶然に依存していた**
- CI では間に合わない。**public リポジトリは push した時点で公開される**ので、コミット時に止めるしかない
- GitHub 内部には旧コミットが残る可能性があり、**一度出したものは完全には消せない**

### clone 後のセットアップ

フックはリポジトリに入るが、`core.hooksPath` はローカル設定なので clone 後に一度必要。

```bash
git config core.hooksPath .githooks
```

検出語（`.githooks/private-l2-patterns`）は git に入らないので、新しいマシンには別途持ち込む。
**無ければフックは黙ってスキップする**（private が無い環境でも動く方針に従う）。
持ち込むまで L2 チェックは効かない。
