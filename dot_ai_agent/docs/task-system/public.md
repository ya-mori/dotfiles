...
```
**保存は必要。不要になったのはマーカー絵文字だけ**（絵文字は 2026-08-12 廃止）。
以前は「保存 ＋ 特定のリアクション」の2段階だったものが、`is:saved` の1段階になった
（どのリアクションを使っていたかは private 側の §5 にある）。
絞り込みは AI の仕分けと `Source` 台帳が肩代わりする。

**`is:saved` を条件から外していないのは、それが「自分が拾うと決めた」唯一の signal だから。**
外すと全チャンネルの全メッセージが候補になり、量が多くてレビューが形骸化する。
...
1. **拾う** — `is:saved after:{2週間前}` ＋ **`to:me after:{2週間前}`**
   - **`is:saved` だけだと、保存し忘れた依頼が構造的に漏れる。**
     実例: 2026-08-18 16:47 の要件定義レビュー依頼（自分宛メンション・保存なし）は
     翌日の棚卸しで拾えず、人から URL を渡されて初めて起票した
   - 自分宛メンションも見れば拾える。**ノイズは増えるが `Source` 突合で処理済みは自動で落ちる**ため、
     実際に仕分ける対象はほぼ未処理分だけ
