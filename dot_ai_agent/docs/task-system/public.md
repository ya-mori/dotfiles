...
実際 {ID prefix}-19 は本文に「2026-08-07 レビュー完了」とあるのに `Updated` は 8/10 に飛んでいた
...
> 実例: 担当者Pさんの「スロット66/67 が description 欠落で 100% 失敗」は
...
- **`Waiting` は種類の位置を保ったまま、同種内では末尾に置く。** Now ビューには出ないが、
  相手から返ってきたらその位置で再開できる（順位を空にすると再開時に埋もれる）
...
- 振り返りは `Status = 'Done'` で絞るため、`Dropped` は自動で集計から外れる
...
|---|---|---|---|---|
| 30 | Status | `Todo` | `Done` | `<repo>#1136` が `merged=2026-08-12T02:30:42Z by=ddhlee`。自分の review は `APPROVED` |
...
「今週やったこと」「今期やったこと」
```
`Status = Done` を `完了日` で絞る。

```sql
WHERE "Status" = 'Done' AND date("date:完了日:start") >= date('YYYY-MM-DD')
```

**レビュー依頼は件数だけ数える。**（タスク名に「レビュー」が入る）
価値を語るのは設計・改善系に絞る。レビューの完了行は「レビュー完了」で足りる。
...
| **見送りを台帳に残すか** | 「検討して見送った」を `Status = Dropped` で載せる案がある。**Slack 起票時の見送りはまだ一度も書き込んでいない**（3回の運用では報告のみで済んだ）。同じものが再度候補に上がって煩わしくなったら入れる。なお `Dropped` 自体は 2026-08-12 に別用途で初使用した（他者が対応して完了した {ID prefix}-25。§5「自分以外が対応して完了したときは `Dropped`」） |
