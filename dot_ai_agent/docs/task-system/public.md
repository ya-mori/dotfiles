...
**このシステムの呼称は「タスク台帳」。** 会話でも Notion 上でもこの語で呼ぶ
（`ore-task` のトリガー文言「タスク台帳を更新して」と揃えてある。ディレクトリ名 `task-system` は slug として残す）。
...
```mermaid
flowchart TD
  A[Slack で依頼に気づく] --> B[検索3本<br/>is:saved / to:me / 自分宛メンション]
...
### select の選択肢を変更するときの注意

**MCP から select / multi_select / status の選択肢リストを更新すると、既存ページの値が黙って消える。**
消えた値は復元できず、MCP はエラーも返さないので検証しないと気づけない。実際に `Project` で踏んでいる。

| やること | なぜ |
|---|---|
| **既存の選択肢を全部列挙したうえで**新しいものを足す | 更新は**リストの全置換**。書かなかった選択肢は消え、それを使っていた全ページの値が飛ぶ |
| 更新後に SQL で**全件検証する** | 消えても無言で通る |
| 消えた値は `update_properties` で**再設定する** | 上書きしかできないため、放置すると欠損が残る |
| **改名だけなら Notion UI でやる** | MCP 経由の改名は「リネーム」ではなく**別 ID の新規作成**になり、旧選択肢を使っていたページの値が null になる。UI なら option ID が維持される |
...
**会話でタスクを参照するときは `Name` を主語にし、ID は補助として添える**（ID だけでは何の話か判断できない）。
表の1列目に ID を置くのは可。
...
```
**2026-08-17 に「後で」保存の運用そのものをやめた。** 依頼に気づいた時点で直接起票する形へ移したため、
`is:saved` は**それ以前に保存した分の拾い残し用**であって、主たる入口ではない。
（マーカー絵文字はさらに前の 2026-08-12 に廃止済み。どのリアクションを使っていたかは private 側の §5 にある）

**主たる入口は「チャンネル上の自分宛メンション」**（手順1の3本目）。
絞り込みは AI の仕分けと `Source` 台帳が肩代わりする。
...
1. **拾う** — 次の3本を `after:{2週間前}` で引く。**3本目を省かない。**
   1. `is:saved` — 保存した分（2026-08-17 以前のものが中心）
   2. `to:me` — **DM / グループ DM しか返らない。チャンネル上の依頼は1件も返らない**
   3. `<@{自分の user id}>` — **チャンネル上の自分宛メンション。ここでしか拾えない依頼がある**
      （user id は private 側の §5 にある）
   - **3本目でしか拾えなかった実例が3回ある。** 2026-08-18 の要件定義レビュー依頼（棚卸しで拾えず、
     人から URL を渡されて初めて起票）、2026-09-01 のレビュー依頼（8日 stale で発見）、
     2026-09-03 の連携可否の問い合わせ（当日 10:00 着・返信ゼロ）
   - **かつてここに「`to:me` を見れば自分宛メンションも拾える」と書いていたが、これは誤りだった。**
     2026-09-01 に検索65件すべてが DM で、チャンネル発言は0件と確定している
   - ノイズは増えるが **`Source` 突合で処理済みは自動で落ちる**ため、実際に仕分ける対象はほぼ未処理分だけ
   - **private 側が無い環境では、Slack のユーザー検索で自分の user id を引いてから3本目を実行する。**
     引けないなら作業を止めて確認する（3本目を省くと構造的に漏れる）
   - レビュー依頼に限れば GitHub の `review-requested:@me` でも拾えるが、
     **Notion / Figma のレビュー依頼は GitHub に出ない**ので Slack 側も要る
...
**この台帳は本人専用で、そのまま人に見せるものではない。**
チームの会の進捗欄に流用するときは、**他の人の判断や作業に影響するか**で絞る。
全件を要約すると、共有すべきものが埋もれる。

原則として出さないもの（**基準に照らして影響するなら、この限りではない**）:

- 人事・評価系（場の性質が違う。具体例は private 側）
- 個人の開発環境の話
- 自分の中で閉じる定常作業
- 他者の PR へのレビュー（相手には既に伝わっている）

**「台帳を見れば分かる」を、共有を省く理由にしない。**
